...
public class UserService implements UserDetailsService {
        ...
        if (optUser.isEmpty()) {
            ...
            if (followingIds.size() > 0) {
                ...
                responseObj.setMessage(
                    "User id " + id + " does not follow anyone"
                );
                responseObj.setPayload(null);
                return responseObj;
            }
        ...
        if (optUser.isEmpty()) {
            ...
            if (followerIds.size() > 0) {
                for (String followerId : followerIds) {
                    Optional<UserEntity> optFollowerUser = userRepo.findById(
                        followerId
                    );
                ...
                responseObj.setStatus("fail");
                responseObj.setMessage(
                    "User id " + id + " does not have any follower"
                );
                responseObj.setPayload(null);
                return responseObj;
            }
    ...
    public ResponseObjectService saveUser(UserEntity inputUser) {
        ...
        if (optUser.isPresent()) {
            responseObj.setStatus("fail");
            responseObj.setMessage(
                "Email address " + inputUser.getEmail() + " existed"
            );
            responseObj.setPayload(null);
            return responseObj;
        } else {
            inputUser.setPassword(
                bCryptEncoder.encode(inputUser.getPassword())
            );
    ...
    public ResponseObjectService update(UserEntity inputUser) {
        ...
        if (optUser.isEmpty()) {
            responseObj.setStatus("fail");
            responseObj.setMessage(
                "user id: " + inputUser.getId() + " not existed"
            );
            responseObj.setPayload(null);
            return responseObj;
        } else {
    ...
    public ResponseObjectService followUser(DoubleIdObjectEntity doubleId) {
        ...
        ResponseObjectService responseObj = new ResponseObjectService();
        Optional<UserEntity> optFollowedUser = userRepo.findById(
            doubleId.getId1()
        );
    ...
    public ResponseObjectService unfollowUser(DoubleIdObjectEntity doubleId) {
        ...
        if (optFollowedUser.isEmpty() || optFollower.isEmpty()) {
            ...
            responseObj.setMessage(
                "User id " +
                follower.getId() +
                " successfully unfollowed user id " +
                followedUser.getId()
            );
            responseObj.setPayload(new IdObjectEntity(doubleId.getId1()));
            return responseObj;
    ...
    public UserDetails loadUserByUsername(String email)
        ...
        if (optUser.isEmpty()) {
            ...
            springUser =
                new User(foundUser.getEmail(), foundUser.getPassword(), ga);
            return springUser;
        }
    }
}
